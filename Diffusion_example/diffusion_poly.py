...
class EquationLearning(object):
    ...
    def __init__(self, N, X, u, f, K_u=RBF_kernel_u, K_f=poly_kernel_f):
    ...
    def get_K_f(self, X1, X2, ls):  # get kernel matrix of f
        ...
        return v_K_f_fun(X1_p, X2_p, ls, 2).reshape((M, N))
    ...
    def learn_K_u(self, u, X):  # interpolating the training solutions
        u = jnp.ravel(u)
    ...
    def learn_K_f(self):  # map the derivatives to the sources
        ...
        num_folders = 10
        rs = np.random.randint(123, size=num_folders)
        ls = np.linspace(0, 0.0007, 10)  # search range
        errs_by_ls = np.zeros((ls.shape[0], num_folders))
        ...
        for j in range(num_folders):
            ...
            for i in range(ls.shape[0]):
                ...
                errs_by_ls[i, j] = ((metrics.mean_squared_error(f_val.reshape(-1), f_val_pred.reshape(-1))))
        errs_by_ls = errs_by_ls.mean(axis=1)
        self.K_f_ls = ls[np.argmin(errs_by_ls)]
        print("Validation set error ", errs_by_ls.min(), " kernel params for f ", self.K_f_ls)
    ...
    def solve_other_source(self, X_tes, tes_u, f):
        ...
        self.N_f = (N_x_tes - 1) * (N_x_tes - 2)  # number of the rest points
