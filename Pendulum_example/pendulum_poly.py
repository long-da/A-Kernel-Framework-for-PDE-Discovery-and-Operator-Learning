...
N_p = 20
data = np.load('pendulum.npy', allow_pickle=True).item()  # load data
tr_f = data['tr_f'][:N_p, :]  # training sources and solutions
tr_s = data['tr_s'][:N_p, :]
te_f = data['te_f']  # test sources and solutions
te_s = data['te_s']
X0 = data['X0']
te_f0 = data['te_f0']
te_s0 = data['te_s0']
...
class EquationLearning(object):
    ...
    def __init__(self, X, u1, u2, f, K_u=RBF_kernel_u_1d, K_f=poly_kernel_f):
        ...
        self.f_jitter = 1e-5  # jitter for kernel matrix of f
        self.u1 = u1  # training solutions
        self.u2 = u2
    ...
    def get_K_f(self, X1, X2, ls):  # get kernel matrix of f
        ...
        v_K_f_fun = vmap(self.K_f.kappa, (0, 0, None, None))
        return v_K_f_fun(X1_p, X2_p, ls, 3).reshape((M, N))
    ...
    def learn_K_f1(self):  # learn the first equation
        validation_ratio = 0.5
        num_folders = 10
        rs = np.random.randint(123, size=num_folders)
        ls = np.linspace(0.01, 0.1, 100)
        errs_by_ls = np.zeros((ls.shape[0], num_folders))
        ...
        print("Validation set error ", errs_by_ls.min(), " kernel params for f ", self.K_f_ls1)
    ...
    def learn_K_f2(self):  # learn the second equation
        validation_ratio = 0.5
        num_folders = 10
        rs = np.random.randint(123, size=num_folders)
        ls = np.linspace(0.001, 0.1, 100)
        errs_by_ls = np.zeros((ls.shape[0], num_folders))
        ...
        print("Validation set error ", errs_by_ls.min(), " kernel params for f ", self.K_f_ls2)
